class MerchantCustomerSupport < ActiveRecord::Base
	...
	validates :telephone, format: {with: /\d[0-9]\)*\z/, message: "Only positive number without spaces are allowed"}
	validates :emergency, format: {with: /\d[0-9]\)*\z/, message: "Only positive number without spaces are allowed"}
