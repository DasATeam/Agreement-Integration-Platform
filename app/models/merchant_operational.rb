class MerchantOperational < ActiveRecord::Base
    ...
    message: "email tidak sesuai format"}, allow_nil: true
	validates :telephone, format {with: /\d[0-9]\)*\z/,
		message: "Only positive number without spaces are allowed"}, allow_nil: true
	validates :emergency, format {with: /\d[0-9]\)*\z/,
		message: "Only positive number without spaces are allowed"}, allow_nil: true
